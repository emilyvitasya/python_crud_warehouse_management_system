...
datasembako = [
    ...
    Sembako(
        "S102", "Minyak Goreng Bimoli 2L", "Minyak", 40, 38000, "1-11-2024", "1 Tahun"
    ),
...
def ShowList(data_sembako):
    ...
    lebar_kolom_min = {"Harga": 15, "Tanggal_Masuk": 15, "Kadaluarsa": 12}
        ...
        for kolom in header:
            ...
            if kolom == "Harga":
                ...
                if isinstance(nilai, date):
                    ...
                    nilai = (
                        datetime.strptime(nilai, "%d-%m-%Y").date().strftime("%d-%m-%Y")
                    )
            else:
                nilai = str(nilai)
    ...
    while True:
        ...
        while True:
            ...
            if len(kode_sembako) >= 4 and kode_sembako.isalnum():
                ...
                print(
                    "Kode tidak valid. Minimal 4 karakter, kombinasi huruf dan angka."
                )
        ...
        konfirmasi = input(
            "Apakah anda yakin data ini akan ditambahkan (Y/T)? "
        ).capitalize()
        if konfirmasi == "Y":
            datasembako.append(
                Sembako(
                    kode_sembako, nama, jenis, stok, harga, tanggal_masuk, kadaluarsa
                )
            )
            tampilkan_data_sembako()
...
Silahkan pilih menu diatas : """)
        if Updatedatacust == "1":
            tampilkan_data_sembako()
            kode_cari = input("\n\tMasukkan Kode Sembako yang ingin diubah: ")
            ...
            if produk_ditemukan:
                ...
                if konfirmasi == "Y":
                    ...
                    if kolom == 1:
                        ...
                        try:
                            masukan_data = datetime.strptime(
                                masukan_data, "%d-%m-%Y"
                            ).date()
                        except ValueError:
                            print("Format tanggal tidak valid. Gunakan DD-MM-YYYY.")
