...
def tambah_produk():
    while True:
        ...
        # Batasan tanggal masuk (Maksimal hari ini, atau kosongkan untuk otomatis hari ini)
        while True:
        ...
        if konfirmasi == 'Y':
            baru = Sembako(kode_sembako, nama, jenis, stok, harga, tanggal_masuk, kadaluarsa)
...
# Fungsi Update Data (Kode tidak bisa diubah + Live Update Preview)
def UpdateData():
    while True:
...
Silahkan pilih menu diatas : ''')        
    ...
    Masukkan kolom data yang ingin diubah (1-6): '''))
                    ...
                    # Mapping ulang karena kolom kode (index 1 lama) sudah dihapus dari pilihan
